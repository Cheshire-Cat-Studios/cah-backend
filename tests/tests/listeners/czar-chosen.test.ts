...
describe('Cards chosen event listener', () => {
    ...
    test('-- Black card has changed', async () => {
        ...
        for (const uuid in GameData.player_data) {
            expect(GameData.player_data[uuid].black_card)
                .toBe(new_black_card)
        }
    })
    ...
    test('-- Czar has changed', async () => {
        ...
        //Test game data
        for (const uuid in GameData.player_data) {
            for (const player of GameData.player_data[uuid].scoreboard) {
    ...
    test('-- Is no longer czar phase', async () => {
        ...
        //Test game data
        for (const uuid in GameData.player_data) {
            expect(GameData.player_data[uuid].is_czar_phase)
                .toBe(false)
        }
    })
    ...
    test('-- Cards in play have been cleared', async () => {
        ...
        //Test game data
        for (const uuid in GameData.player_data) {
            expect(GameData.player_data[uuid].cards_in_play)
                .toStrictEqual({})
        }
    })
