...
class LeaveAction extends GameAction {
	async handle(): Promise<void> {
		...
		// await this.redis.del(this.getPlayerRedisKey('is_active'))
		await this.redis.del(this.getPlayerRedisKey('deck'))
		await this.redis.del(this.getPlayerRedisKey('hand'))
		...
		const
			...
			generic_emit_data = {
				scoreboard,
				is_czar_phase,
				player_who_left_name: user.row.name,
				cards_in_play,
			}
